...
import { useTakeSnapshot } from "../api/queries";
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="text-[12.5px] text-dim mt-1">
            {fa ? `${online} فعال · ${offline} قطع` : `${online} online · ${offline} offline`}
          </div>
        </div>
            ...
            <Panel key={c.id}>
              <div className="relative" style={{ aspectRatio: "16/9", background: "#05080F" }}>
                ...
                <div className="absolute bottom-[9px] flex gap-1.5" style={{ insetInlineEnd: 9 }}>
                  ...
                  <button
                    onClick={() => takeSnapshot.mutate(c.id)}
              ...
              <div className="px-[13px] py-[11px] flex items-center gap-2.5">
                <div className="flex-1 min-w-0">
                  <div className="text-[12.5px] font-medium text-txt overflow-hidden text-ellipsis whitespace-nowrap">{c.name}</div>
                </div>
                {c.zone && <span className="text-[10.5px] px-2 py-[3px] rounded-md bg-panel2 text-dim">{c.zone}</span>}
              </div>
            </Panel>
          ))}
        </div>
      ...
      {view === "list" && (
        <Panel>
          <div
            ...
            style={{ gridTemplateColumns: "2fr .9fr .9fr .7fr", letterSpacing: ".12em" }}
          >
          ...
          {cams.map((c, i) => (
            <div
              ...
              style={{ gridTemplateColumns: "2fr .9fr .9fr .7fr", background: i % 2 ? "var(--fv-zebra)" : "transparent" }}
            >
